...
function paintorbit() {
  ...
  $.post('/keplercalen', { 'e': e, 'I_0': I_0, 'sqrt_A': sqrt_A, 'OMEGA': OMEGA, 'w': w, 'M0': M0 }, function (res) {
    console.log(res)
...
//   }
// });
function getascend(ascendingNodeAltitude, sat) {
  if (point != '') {
  ...
  var longitudeOfAscendingNode = document.getElementById('OMEGA_value').value;
  ...
  // 计算近地点经纬度
  const perigeeLongitude = calculatePerigeeLongitude(longitudeOfAscendingNode, ω);
  ...
  var equator = new Cesium.Entity({
    ...
    ellipsoid: {
      ...
      material: Cesium.Color.fromBytes(135,206,250).withAlpha(0.3),
    }
  });
  ...
  var acendpos = Cesium.Cartesian3.fromDegrees(parseFloat(longitudeOfAscendingNode) + 0.0125, 0, ascendingNodeAltitude / 8);
  var midpos = Cesium.Cartesian3.fromDegrees(parseFloat(longitudeOfAscendingNode) / 2, 0, (semimajoraxis - 6378137 + ascendingNodeAltitude) / 16);
  var spos = Cesium.Cartesian3.fromDegrees(0, 0, (semimajoraxis - 6378137) / 8);
  ...
  if (sat != '') {
    ...
    var satheight = cartographic.height;
    var satlon = calculatePerigeeLongitude(longitudeOfAscendingNode, parseFloat(ω) + parseFloat(E));
    var satlat = calculatePerigeeLatitude(inclination, parseFloat(ω) + parseFloat(E));
...
const ascendingNodeAltitude = calculateAscendingNodeAltitude((document.getElementById('sqrt_A_value').value) * (document.getElementById('sqrt_A_value').value), document.getElementById('e_value').value, 6371000);
getascend(ascendingNodeAltitude, '')
