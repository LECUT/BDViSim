...
function painDOP(xdata) {
  ...
  option = {
    ...
    legend: {
      ...
      icon: 'circle', // 图例的图标形状，可以是 'circle', 'rect', 'roundRect', 'triangle', 'diamond', 'pin', 'arrow', 'none' 等
      itemWidth: 10, // 图例的图标宽度
      itemHeight: 10, // 图例的图标高度
      // itemGap: 20, // 图例项之间的间距
      orient: 'horizontal', // 图例的布局方向，可以是 'horizontal'（水平）或 'vertical'（垂直）
      top: '0px', // 图例相对于容器上侧的偏移量，可以是像素值或百分比
      left: 'center', // 图例相对于容器左侧的偏移量，可以是像素值或百分比
    },
    xAxis: {
      name: 'Time',
      scale: true,
...
function painvisibility(sat, xdata) {
  ...
  for (var i in sat) {
    ...
    // var satdata=i.substring(1,2)
    // for (var j = 0; j < sat[i]['hH'].length; j++) {
    //   if (sat[i]['hH'][j] != null) {
    //     sat[i]['hH'][j] = parseInt(i.substr(1, 2))
    //   }

    //   // console.log(i,parseInt(i.substr(1,2)))
    // }
